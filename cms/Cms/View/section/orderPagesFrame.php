...
<head>
	...
	function saveneworden()
	{
		...
		$.post('<?php Core_Common_Route::linkController('page:saveorder', array('parent_type' => Cms_Model_MenuItem::PARENT_SECTION, 'parent_id' => $currentparent->get('id'), 'site_id' => $site->get('id')));?>',
...
<body>
	...
	<table border="0" cellspacing="0" cellpadding="0" id="pages_list">
		<thead>
		<tr><th>Nombre de la página</th></tr>
		</thead>
		<tbody>
			<?php if($pages->count() > 0): ?>
			<?php foreach($pages as $page): ?>
			<tr id="<?php echo $page->get('id'); ?>"><td><?php echo $page->get('id').' -  '.$page->get('name'); ?></td></tr>
			<?php endforeach; ?>
			<?php endif; ?>
		</tbody>
	</table>
